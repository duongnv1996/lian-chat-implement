...
public class ChattingRemoteImpl extends Interactor implements ChattingContract.Interactor {
    ...
    public void getRoomInfo(String id, int index, final boolean isLoadmore) {
        ...
        getmService().getRoomGroupChatInfo(profile.getId(), id, index).enqueue(new CallBackBase<ApiResponse<Room>>() {
            @Override
    ...
    @Override
    public void sendMessage(final String sendToId, String message, final SocketClient socketClient, final int positionMessage, final String listUserIds) {
        Profile profile = AppController.getInstance().getmProfileUser();
        if (profile == null) {
            listener.onErrorAuthorization();
            return;
        }
        getmService().chatGroup(profile.getId(), sendToId, message).enqueue(new CallBackBase<ApiResponse<Message>>() {
            @Override
            public void onRequestSuccess(Call<ApiResponse<Message>> call, Response<ApiResponse<Message>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    if (response.code() == AppConstant.CODE_API_SUCCESS) {
                        listener.onSuccessSendMessage(sendToId, socketClient, response.body().getData(), positionMessage, listUserIds);
                    } else {
                        listener.onError(response.body().getMessage());
                    }
                } else {
