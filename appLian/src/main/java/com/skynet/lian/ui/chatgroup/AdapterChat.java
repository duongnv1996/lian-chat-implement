...
public class AdapterChat extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    Context context;
    com.skynet.lian.ui.chatting.AdapterChat.CallBackChat onResponse;
    String ava_user;
    String ava_driver;
    int layoutId;
    private final int VIEW_PROG = 3;
    com.skynet.lian.ui.chatting.AdapterChat.CallBackChat iCallback;
    ...
    public AdapterChat(List<Message> list, Context applicationContext, RecyclerView recyclerView, com.skynet.lian.ui.chatting.AdapterChat.CallBackChat iCallback) {
        this.mMessages = list;
...
//                        }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == AppConstant.TYPE_USER) {
            ...
            return new ViewHolerMessageCommom(LayoutInflater.from(parent.getContext()).inflate(R.layout.textview_chat_commom, parent, false));
...
//                }
                ...
                try {
//                    holder.contentTxt.setText(URLDecoder.decode(message.getContent(), "UTF-8"));
                    String content =    message.getContent().replaceAll("\\+","```");
                    holder.contentTxt.setText(URLDecoder.decode(content, "UTF-8").replaceAll("```","+"));
...
//                            if((message.getFile().contains("http")))
        ...
        } else if(holderRaw instanceof ViewHolerMessageCommom){
            ViewHolerMessageCommom holder = (ViewHolerMessageCommom) holderRaw;
            if(mMessages.get(position)!=null)
            holder.tvCommom.setText(mMessages.get(position).getContent());
        } else{
...
//
//            }else{
            if(mMessages.get(position).getType()==3) return 3;
...
//            }
    ...
    public class ViewHolerMessageCommom extends RecyclerView.ViewHolder {

        @BindView(R2.id.tvCommom)
        TextView tvCommom;

        public ViewHolerMessageCommom(View itemView) {
            super(itemView);
            ButterKnife.bind(this, itemView);
        }
    }
