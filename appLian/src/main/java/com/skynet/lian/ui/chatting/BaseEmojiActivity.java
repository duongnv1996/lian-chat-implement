...
import com.skynet.lian.ui.base.ActivityNetwork;
import com.skynet.lian.ui.main.MainActivity;
import com.skynet.lian.ui.splash.SplashActivity;
import com.skynet.lian.ui.views.DialogOneButtonUtil;
...
public abstract class BaseEmojiActivity extends EmojiCompatActivity {
    protected static final String TAG = BaseEmojiActivity.class.getName();
...
//    }
        ...
        public void onReceive(Context context, Intent intent) {
            if (!NetworkUtils.isConnected()) {
                if (!AppController.getInstance().isToogleInternet()) {
                    startActivity(new Intent(BaseEmojiActivity.this, ActivityNetwork.class));
                }
            } else {
    ...
    ServiceConnection mConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            BaseEmojiActivity.this.mBounded = true;
            SocketClient.LocalBinder mLocalBinder = (SocketClient.LocalBinder) service;
            BaseEmojiActivity.this.mSocket = mLocalBinder.getServerInstance();
            onSocketConnected();
        }

        @Override
        public void onServiceDisconnected(ComponentName name) {
            BaseEmojiActivity.this.mBounded = false;
            BaseEmojiActivity.this.mSocket = null;
        }
    };
...
//        if (this instanceof MainActivityLian) {
//
//        } else {
//            isBackPressed = true;
//        }
    ...
    public void showDialogExpired() {
        if (dialogOneButtonUtil == null) {
            ...
            dialogOneButtonUtil.setDialogOneButtonClick(new DialogOneButtonUtil.DialogOneButtonClickListener() {
                @Override
                public void okClick() {
                    Intent intent = new Intent(BaseEmojiActivity.this, MainActivity.class);
...
//    }
    ...
    public void logOut() {
        Intent intent = new Intent(BaseEmojiActivity.this, SplashActivity.class);
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        intent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
        AppController.getInstance().setmProfileUser(null);
        AppController.getInstance().getmSetting().remove(AppConstant.KEY_PROFILE);
    ...
    public void clearTaskAndGo(Context packageContext, Class<?> cls) {
        Intent intent = new Intent(BaseEmojiActivity.this, cls);
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        intent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
        startActivity(intent);
        finishAffinity();
