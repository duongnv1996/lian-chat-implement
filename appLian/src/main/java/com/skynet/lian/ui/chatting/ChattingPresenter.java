...
//    }
    ...
    @Override
    public void sendMessage(String sendToId, String message, SocketClient socketClient, int positionMessage) {
        if (isAvaliableView()) {
            interactor.sendMessage(sendToId, message, socketClient, positionMessage);
        }
    }
...
//    }
    ...
    @Override
    public void onSuccessSendMessage(String sendToId, SocketClient socketClient, Message messageObject, int positionMessage) {
        if (isAvaliableView()) {
            view.hiddenProgress();
            if(socketClient != null){
                socketClient.sendMessage(messageObject.getContent(),AppController.getInstance().getmProfileUser().getId(),sendToId,messageObject.getChat_id(),AppConstant.SINGLE);
            }
...
//        view.onSuccessSendMessage(message);
    ...
    public void onSuccuessUploadImages(List<Message> message, int positionMessage, SocketClient socketClient, String idRoom) {
        if (isAvaliableView()) {
            view.hiddenProgress();
            if(socketClient != null){
                socketClient.sendMessage("Hình ảnh",AppController.getInstance().getmProfileUser().getId(),"",idRoom,AppConstant.SINGLE);
            }
    ...
    public void onSuccuessUploadFiles(Message url, int positionMessage, SocketClient socketClient, String idRoom) {
        if (isAvaliableView()) {
            view.hiddenProgress();
            if(socketClient != null){
                socketClient.sendMessage("Tệp tin",AppController.getInstance().getmProfileUser().getId(),"",idRoom,AppConstant.SINGLE);
            }
